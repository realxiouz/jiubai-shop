...
export default {
  ...
  mutations: {
    ...
    changeCount(state, {shop_id, sku, amount}) {
      let newArr = JSON.parse(JSON.stringify(state.cartGoods))
      let curShop = newArr.find(i => i.shop_id == shop_id)
      let cur = curShop.list.find(i => i.sku == sku)
      cur.amount = amount
  ...
  getters: {
    total(state, getters, rootState) {
      ...
      let key = rootState.user.info.is_soldier ? 'soldier_price' : 'price'
      return arr.filter(i => i.is_choose).reduce((pre, cur) => pre + cur.amount * cur[key], 0).toFixed(2)
    },
    isAllChoose(state) {
